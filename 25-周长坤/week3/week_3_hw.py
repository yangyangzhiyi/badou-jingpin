...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab):
        ...
        # self.pool = nn.AvgPool1d(sentence_length)               # 池化层 缩减维度，提高计算效率
        self.rnn = nn.RNN(vector_dim, sentence_length, bias=False, batch_first=True)
...
def build_vocab():
    ...
    vocab['unk'] = len(vocab)               # 词表中没有的词归于此类
    return vocab
...
def main():
    ...
    plt.show()
    # 保存模型
    torch.save(model.state_dict(), "week_3_hw.pth")
    # 保存词表
...
if __name__ == "__main__":
    # main()
    test_strings = ["favfee", "wbsdfg", "rqwdeg", "nrk六ww", "0qavvo", "rr$cop"]
    predict("week_3_hw.pth", "vocab.json", test_strings)
