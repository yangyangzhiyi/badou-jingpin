...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, vocab):
        ...
        # 设置 padding_idx=0 ，即不去训练词表的 0 位置。0 作为后补位符存在
        self.embedding = nn.Embedding(len(vocab), vector_dim, padding_idx=0)  # embedding层
        #self.pool = nn.AvgPool1d(sentence_length)    # 池化层
        #self.classify = nn.Linear(vector_dim, 1)     # 线性层
        self.rnnlayer = nn.RNN(vector_dim, 1, bias=False, batch_first=True)
...
def evaluate(model, vocab, sample_length):
    ...
    with torch.no_grad():
        y_pred = model(x)      # 模型预测
        #print(y_pred)
        for y_p, y_t in zip(y_pred[0], y):  # 与真实标签进行对比
            #print(y_p)
            #print(y_t)
...
def main():
    ...
    # 保存词表
    writer = open("work-vocab.json", "w", encoding="utf8")
...
if __name__ == "__main__":
    ...
    predict("work-model.pth", "vocab.json", test_strings)
