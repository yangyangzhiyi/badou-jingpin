...
class TorchModel(nn.Module):
    ...
    def forward(self, x, target=None):
        if self.use_bert:  # bert返回的结果是 (sequence_output, pooler_output)
            sequence_output, pooler_output = self.encoder(x)
        else:
            x = self.embedding(x)  # input shape:(batch_size, sen_len, embedding_size)
        ...
        if target is not None:
            if self.use_bert:
                return self.loss(predict.view(predict.shape[0], -1), target.squeeze())
            else:
