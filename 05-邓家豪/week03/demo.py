...
class TorchModel(nn.Module):
    def __init__(self, vocab, hidden_size, vector_dim):
        super().__init__()
        self.embedding = nn.Embedding(len(vocab), vector_dim)  # embedding层 vector_dim：每个vocab的向量维度
        self.rnn = nn.RNN(input_size=vector_dim, hidden_size=vector_dim, bias=False, batch_first=True)
    ...
    def forward(self, x, y=None):
        ...
        _, x = self.rnn(x)  # 使用下划线(_)忽略了RNN的h数组输出 , ht:[]
        x = x.squeeze()
        x = self.classify(x)  # (20,6)
