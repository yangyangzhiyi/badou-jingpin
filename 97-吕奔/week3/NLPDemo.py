...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab):
        ...
        self.rnn = nn.rnn(vector_dim, vector_dim, bias=True, batch_first=True)
...
def build_vocab():
    ...
    vocab['unk'] = len(vocab) #27 ，下标26
    vocab['pad'] = len(vocab) #28，下标27
    return vocab
...
def build_sample(vocab, sentence_length):
    ...
    #指定哪些字出现时为正样本
    if set("abc") & set(x):# 跟abc有交集，a或b或c
        y = 1
    #指定字都未出现，则为负样本
    else:
        y = 0
...
if __name__ == "__main__":
    main()
    #test_strings = ["fnvfee", "wzsdfg", "rqwdeg"]
