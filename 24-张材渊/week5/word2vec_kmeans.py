...
def main():
    ...
    for label, vectors in vector_label_dict.items():
        distances = [get_distance(kmeans.cluster_centers_[label], vector) for vector in vectors]
        label_distance_dict[label] = len(distances) and sum(distances) / len(distances)

    distance_desc_list = sorted(label_distance_dict.items(), key=lambda x: x[1])

    for label, distance in distance_desc_list[:10]:
