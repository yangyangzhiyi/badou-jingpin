...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        x = self.layer(x)  # (batch_size, vector_dim, sen_len)->(batch_size, vector_dim, 1)
        x = x[1]  # (batch_size, vector_dim, 1) -> (batch_size, vector_dim)
        x = self.classify(x)  # (batch_size, vector_dim) -> (batch_size, 1)
        y1 = self.activation(x)
        y_pred=y1[0]# (batch_size, 1) -> (batch_size, 1)
        if y is not None:
...
# 建立模型
def build_model(vocab, char_dim, char_hidden):
    model = TorchModel(char_dim, char_hidden, vocab)
    return model
...
def main():
    ...
    vocab = build_vocab()
    # 建立模型
    model = build_model(vocab, char_dim, char_hidden)
    # 选择优化器
...
def predict(model_path, vocab_path, input_strings):
    ...
    model = build_model(vocab, char_dim, char_hidden)  # 建立模型
