...
class TorchModel(nn.Module):
    ...
    def forward(self, inputs, target=None):
        ...
        if target is not None:
            ...
            # target_onehot = nn.functional.one_hot(target, num_classes=self.class_num)
            # return self.loss(predict, target_onehot.squeeze().to(torch.float32))
            # return self.loss(predict.squeeze(), target.squeeze().to(torch.float32))
        else:
            return predict
