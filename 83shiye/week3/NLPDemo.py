...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        _, h = self.rnn(x)                           #(batch_size, vector_dim, sen_len)->(batch_size, vector_dim, 1)
        # print(x.shape, h[0].shape)
        # x = x.squeeze()                            #(batch_size, vector_dim, 1) -> (batch_size, vector_dim)
        x = self.classify(h[-1])                       #(batch_size, vector_dim) -> (batch_size, 1)
...
if __name__ == "__main__":
    ...
    print(f"正确结果: {[int(bool(set(item) & set('abc'))) for item in test_strings]}")
