...
class TorchModel(nn.Module):
    def __init__(self, input_dim, hidden_size, num_rnn_layers, vocab, num_classes):
        super(TorchModel, self).__init__()
        self.num_classes = num_classes
        self.bert = BertModel.from_pretrained(r"bert-base-chinese", return_dict=False)
        self.classify = nn.Linear(hidden_size, self.num_classes)
...
def main():
    ...
    model = TorchModel(char_dim, hidden_size, num_rnn_layers, vocab, 2)   #建立模型
