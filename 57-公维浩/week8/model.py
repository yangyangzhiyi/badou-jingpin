...
class SentenceEncoder(nn.Module):
    ...
    # 输入为问题字符编码
    def forward(self, x):  # x:(batch_size, sen_len)
        # sentence_length = torch.sum(x.gt(0), dim=-1)
        x = self.embedding(x)  # x:(batch_size, sen_len, hidden_size)
        # 使用lstm
        x, _ = self.lstm(x)  # x:(batch_size, sen_len, hidden_size)
        # 使用线性层
        x = self.layer(x)  # x:(batch_size, sen_len, hidden_size)
        # x:(batch_size, sen_len, hidden_size)->  # x:(batch_size, hidden_size, sen_len)->x:(batch_size, hidden_size, 1)->x:(batch_size, hidden_size)
