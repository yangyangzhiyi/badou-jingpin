...
class LanguageModel(nn.Module):
    def __init__(self, input_dim, vocab):
        ...
        self.bert = BertModel.from_pretrained(r"E:\deep_learn\bert-base-chinese",
                                              return_dict=False)
    ...
    def forward(self, x, y=None):
        ...
        # x = pooler_output[:, -1, :]        #output shape:(batch_size, input_dim)
        # x = self.dropout(x)
        y_pred = self.classify(pooler_output)  # output shape:(batch_size, input_dim)
        if y is not None:
