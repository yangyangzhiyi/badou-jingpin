...
def build_sample():
    ...
    if x[0]<x[1]+x[2] and x[1]<x[0]+x[2] and x[2]<x[0]+x[1]:
        if x[0]==x[1]==x[2]:
            ...
            if x[0]*x[0]+x[1]*x[1]==x[2]*x[2] or x[0]*x[0]+x[2]*x[2]==x[1]*x[1] or x[1]*x[1]+x[2]*x[2]==x[0]*x[0]:
                y[1] = 1
                return x ,y
            else:
                y[2] = 1
                return x ,y
        elif x[0]*x[0]+x[1]*x[1]==x[2]*x[2] or x[0]*x[0]+x[2]*x[2]==x[1]*x[1] or x[1]*x[1]+x[2]*x[2]==x[0]*x[0]:
            y[3]=1
            return x ,y
        else:
            y[4] = 1
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            ...
            flag = 0
            for y1,y2 in zip(y_p,y_t):
                if (y2 == 0 and y1 <= 0) or (y2==1 and y1 >= 1):
                    flag = 1
                    continue
                flag = 0
                break
...
def main():
    ...
    # torch.set_printoptions(profile="full")    #输出整个Tensor
    # print(train_x)
    # print(train_y)
    # 训练过程
