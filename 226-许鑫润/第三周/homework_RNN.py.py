...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab, hidden_size): #vector 是字表
    ...
    def forward(self, x, y=None):
        ...
        _, x = self.layer(x)                       # _ (batch_size x sen_len x hidden_size)
                                                   # x (1 x batch_size x hidden_size)
        x = x.squeeze() # (batch_size x hidden_size) 也可用view降维
        x = self.classify(x)                       #(batch_size, hidden_size) -> (batch_size, 1)
...
if __name__ == "__main__":
    ...
    hidden_size = 27
    predict("model.pth", "vocab.json", test_strings, hidden_size)
